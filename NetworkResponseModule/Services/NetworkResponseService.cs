...
namespace NetworkResponseModule.Services
{
    ...
    class NetworkResponseService : INetworkResponseService
    {
        ...
        public void GetNetWorkResponse(string hostip, int timeout, int PingTimout, int max_ttl, int iteration_interval, int iterations_per_host, int packet_size,mtrRoutes mtrRoutes, result result)
        {
            ...
            //public bool Execute(string hostOrIpAddress, int timeout, int max_ttl, int iteration_interval, int iterations_per_host, int packet_size, int waiting, mtrRoutes mtrRoutesRes, mtrResult mtr)
             mtrCommand.Execute(hostip, PingTimout, timeout, max_ttl, iteration_interval, iterations_per_host, packet_size,(IEnumerable<IPAddress> routes1) =>
            {

                mtrRoutes(routes1);


            }, (int loopInternalCount, int loopCount, string host, int Loss,
